...
public class EarlyAlertServiceImpl extends // NOPMD
    ...
    public static final String MESSAGE_CREATED_FOR_EARLY_ALERT = "Message {} created for EarlyAlert {}";
    public static final String EARLY_ALERT_PERSON_IS_MISSING = "EarlyAlert.Person is missing.";
    public static final String TERM_TO_REPRESENT_EARLY_ALERT = "term_to_represent_early_alert";
    private final transient EarlyAlertDao dao;
    private final transient ConfigService configService;
    private final transient EarlyAlertRoutingService earlyAlertRoutingService;
    private final transient MessageService messageService;
    private final transient MessageTemplateService messageTemplateService;
    private final transient EarlyAlertReasonService earlyAlertReasonService;
    private final transient EarlyAlertSuggestionService earlyAlertSuggestionService;
    private final transient PersonService personService;
    private final transient FacultyCourseService facultyCourseService;
    private final transient TermService termService;
    private final transient PersonProgramStatusService personProgramStatusService;
    private final transient ProgramStatusService programStatusService;
    private final transient StudentTypeService studentTypeService;
    private final transient SecurityService securityService;
    private final transient EarlyAlertSearchResultTOFactory searchResultFactory;
    private final transient EarlyAlertResponseReminderRecipientsConfig earReminderRecipientConfig;
    private final transient EnrollmentStatusService enrollmentStatusService;

    private static final Logger LOGGER = LoggerFactory
            .getLogger(EarlyAlertServiceImpl.class);

    @Autowired
    public EarlyAlertServiceImpl(ConfigService configService, EarlyAlertDao dao, EarlyAlertRoutingService earlyAlertRoutingService, MessageService messageService, StudentTypeService studentTypeService, MessageTemplateService messageTemplateService, EarlyAlertReasonService earlyAlertReasonService, EarlyAlertResponseReminderRecipientsConfig earReminderRecipientConfig, EarlyAlertSuggestionService earlyAlertSuggestionService, PersonService personService, FacultyCourseService facultyCourseService, ProgramStatusService programStatusService, TermService termService, EnrollmentStatusService enrollmentStatusService, SecurityService securityService, PersonProgramStatusService personProgramStatusService, EarlyAlertSearchResultTOFactory searchResultFactory) {
    ...
    private void sendMessageToAdvisor(@NotNull final EarlyAlert earlyAlert, // NOPMD
        ...
        if (person == null) {
            ...
            final Message message = messageService.createMessage(person, org.springframework.util.StringUtils.arrayToCommaDelimitedString(watcherEmailAddresses
                    ...
                    subjAndBody);
            LOGGER.info(MESSAGE_CREATED_FOR_EARLY_ALERT, message, earlyAlert);
        }
    ...
    public void sendMessageToStudent(@NotNull final EarlyAlert earlyAlert)
            ...
            ValidationException {
        if (earlyAlert.getPerson() == null) {
            throw new IllegalArgumentException(EARLY_ALERT_PERSON_IS_MISSING);
        }
    ...
    public Map<String, Object> fillTemplateParameters(
		...
		if (((earlyAlert.getCreatedBy() == null)
				|| (earlyAlert.getCreatedBy().getFirstName() == null)) && earlyAlert.getCreatedBy() == null) {
			throw new IllegalArgumentException(
					"EarlyAlert.CreatedBy is missing.");
		}
        ...
        try {
            ...
            LOGGER.error(String.format("Early Alert Creator Not found sending message for early alert:%s", earlyAlert.getId()), exp);
        }
        ...
        if (eaMTO.getCoach() == null) {
            try {
                ...
                LOGGER.error(format("Early Alert with id: %s does not have valid campus coordinator, no coach assigned: %s", earlyAlert.getId(), earlyAlert.getCampus().getEarlyAlertCoordinatorId()), exp);
            }
        }
    ...
    public void sendAllEarlyAlertReminderNotifications() {
        ...
        for (Map.Entry<UUID, List<EarlyAlertMessageTemplateTO>> coach : easByCoach.entrySet()) {
            Map<String, Object> messageParams = new HashMap<>();

            coach.getValue().sort((Comparator<EarlyAlertTO>) (p1, p2) -> {
            ...
            for (EarlyAlertMessageTemplateTO ea : coach.getValue()) {
            ...
            messageParams.put("coach", coaches.get(coach.getKey()));
            ...
            try {
                messageService.createMessage(coaches.get(coach.getKey()), null, subjAndBody);
            } catch (Exception exp) {
                LOGGER.error(format("Unable to send reminder emails to coach: %s", coaches.get(coach.getKey()).getFullName()), exp);
            }
        }
    ...
    private EarlyAlertMessageTemplateTO createEarlyAlertTemplateTO(EarlyAlert earlyAlert) {
        ...
        try {
            ...
            LOGGER.error(format("Early Alert with id: %s does not have valid creator: %s", earlyAlert.getId(), earlyAlert.getCreatedBy()), exp);
        }
